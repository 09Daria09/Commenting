// Commenting.cpp : Defines the entry point for the application.
//
#include "framework.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_COMMENTING, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_COMMENTING));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_COMMENTING));//A handle to the class icon. If NULL systems sets an icon by default.
    ...
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_COMMENTING); //Sets the resource name of the class menu, as the name appears in the resource file. 
    ...
    case WM_COMMAND://If the user selects a command item from a menu such as "About" or "Help".
        {
            ...
            case IDM_ABOUT:  //If it's about option
                ...
                break;
            case IDM_EXIT:
                DestroyWindow(hWnd);//If it's EXIT
...
INT_PTR CALLBACK About(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) //About box
{
    ...
    UNREFERENCED_PARAMETER(lParam);  //Checks the message.
    {
        switch (message)
        {
        case WM_INITDIALOG:
            return (INT_PTR)TRUE; //Message for initializing controls and performing any other initialization tasks that affect the appearance of the dialog box.

        case WM_COMMAND:
            if (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDCANCEL)//Sent when the user selects a command item from a menu
            {
                EndDialog(hDlg, LOWORD(wParam));//Checks if the about is closed by any way
                return (INT_PTR)TRUE;//Closes "About" information
            }
            break;
        }
